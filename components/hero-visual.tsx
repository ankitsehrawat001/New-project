...
export function HeroVisual() {
  return (
    <div className="relative min-h-[420px] overflow-hidden rounded-lg border border-white/10 bg-black/30 shadow-neon">
      <div className="absolute inset-0 bg-aurora opacity-80" />
      <div className="absolute inset-8 rounded-lg border border-white/15 bg-white/[.04] backdrop-blur-md" />
      ...
      {techIcons.map((Icon, index) => (
        <motion.div key={index} animate={{ y: [0, -16, 0] }} transition={{ duration: 3 + index, repeat: Infinity }} className="absolute rounded-lg border border-white/10 bg-white/10 p-4 backdrop-blur-xl" style={{ left: `${18 + index * 18}%`, top: `${18 + (index % 2) * 48}%` }}>
